...
export default async function DuplicatesPage({ searchParams }: { searchParams?: Promise<DuplicateParams> }) {
  ...
  const data = dataMode === 'demo'
    ? (() => {
        const facilities = getDemoFacilities();
        const side = (index: number) => ({
          id: facilities[index].facilityId,
          name: facilities[index].facilityName,
          city: facilities[index].city,
          phone: '(207) 555-0100',
          postalCode: '04103',
          latitude: 43.68,
          longitude: -70.29,
          specialties: facilities[index].specialties,
          verificationCount: index + 2,
          callCount: index + 5,
          version: 0,
        });
        return { rows: [{ id: 'demo-candidate', confidence: 'exact' as const, score: 96, reasons: ['Same phone', 'Same ZIP', 'Same coordinates'], decision: 'pending' as const, left: side(0), right: side(1) }], total: 1, page: 1, pageSize: 20 };
      })()
    : await listDuplicateCandidates(principal, page);
  const totalPages = Math.max(1, Math.ceil(data.total / data.pageSize));
  return (
    <AppShell user={principal} dataMode={dataMode}>
      <PageHeader eyebrow="System" title="Duplicate review" summary="Compare likely matches before deciding whether records should remain separate or be merged." meta={dataMode === 'database' ? <DuplicateRefreshButton /> : undefined} />
        ...
        <section key={candidate.id} className="panel p-4" aria-labelledby={`duplicate-${candidate.id}`}>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {[candidate.left, candidate.right].map((side) => (
              <div key={side.id} className="rounded border border-slate-300 p-4">
                {dataMode === 'database'
                  ? <Link className="font-semibold text-slate-950 underline-offset-2 hover:underline" href={`/facilities/${side.id}`}>{side.name}</Link>
                  : <span className="font-semibold text-slate-950">{side.name}</span>}
                <dl className="mt-3 grid grid-cols-[8rem_1fr] gap-x-3 gap-y-2 text-sm">
                  <dt className="text-slate-500">City</dt><dd>{side.city}</dd>
                  <dt className="text-slate-500">Phone</dt><dd>{side.phone || 'Missing'}</dd>
                  <dt className="text-slate-500">ZIP</dt><dd>{side.postalCode || 'Missing'}</dd>
                  <dt className="text-slate-500">Coordinates</dt><dd>{side.latitude !== null && side.longitude !== null ? `${side.latitude.toFixed(4)}, ${side.longitude.toFixed(4)}` : 'Missing'}</dd>
                  <dt className="text-slate-500">Specialties</dt><dd>{side.specialties}</dd>
                  <dt className="text-slate-500">History</dt><dd>{side.verificationCount} verifications · {side.callCount} calls</dd>
                </dl>
              </div>
            ))}
          </div>
          {dataMode === 'database' ? <DuplicateReviewActions candidateId={candidate.id} left={candidate.left} right={candidate.right} /> : null}
        </section>
