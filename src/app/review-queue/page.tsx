...
export default async function ReviewQueuePage({ searchParams }: { searchParams?: Promise<ReviewSearchParams> }) {
  ...
  let data: {
    rows: Array<{ facilityId: string; facilityName: string; city: string; priority: { score: number; reasons: string[] }; acceptingFreshness: { state: string; ageDays: number | null }; availabilityReviewDueAt?: Date | null; assignment: { assignedName: string | null } | null }>;
