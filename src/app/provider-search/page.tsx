    ...
    <AppShell user={principal} dataMode={dataMode} statusMessage={!state.ok ? state.message : null}>
      ...
      <form method="get" action="/provider-search" className="filter-bar" aria-label="Provider search filters">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="form-label">Urgent referral<select name="urgentReferral" defaultValue={urgentReferral ?? ''} className="form-control"><option value="">Any status</option><option value="yes">Accepted</option><option value="no">Not accepted</option><option value="unknown">Unknown</option></select></label>
          ...
          <div className="table-scroll"><table className="data-table min-w-[76rem]">
            ...
            <tbody>{results.map((result) => (
              <tr key={result.facilityId}>
                <td><Link className="font-semibold text-slate-950 underline-offset-2 hover:underline" href={`/facilities/${result.facilityId}`}>{result.facilityName}</Link><span className="block text-xs text-slate-500">{result.city}{result.stateCode ? `, ${result.stateCode}` : ''} · {result.specialties}</span></td>
                <td><ul className="space-y-1 text-sm">{result.matchReasons.slice(0, 4).map((reason) => <li key={reason}>· {reason}</li>)}</ul></td>
                <td><StatusBadge tone={statusTone(result.acceptingStatus)}>Accepting: {humanizeKey(result.acceptingStatus)}</StatusBadge><span className="mt-2 block text-xs text-slate-600">Scheduling: {humanizeKey(result.schedulingStatus)}</span><span className="mt-1 block text-xs text-slate-600">Urgent: {humanizeKey(result.urgentReferralStatus)}</span></td>
